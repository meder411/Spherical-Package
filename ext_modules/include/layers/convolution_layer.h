...
torch::Tensor ConvBackwardInput(torch::Tensor grad_output, torch::Tensor weight,
                                int inputHeight, int inputWidth, int kernel_h,
                                int kernel_w, int stride_h, int stride_w,
                                int pad_h, int pad_w, int dilation_h,
                                int dilation_w);

torch::Tensor ConvBackwardWeight(torch::Tensor grad_output, torch::Tensor input,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("conv_forward", &spherical::ConvForward, "Forward convolution");
  m.def("conv_backward", &spherical::ConvBackward, "Backward convolution");
}
