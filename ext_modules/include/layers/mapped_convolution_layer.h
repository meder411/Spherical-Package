...
torch::Tensor MappedConvForward(torch::Tensor input, torch::Tensor sample_map,
  ...
  if (input.is_cuda()) {
    ...
    CHECK_CUDA(weight);
    CHECK_CUDA(bias);
    return cuda::MappedConvForward(input, sample_map, weight, bias, kernel_size,
                                   interpolation);
  } else
#endif
  {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("mapped_conv_forward", &spherical::MappedConvForward,
        "Forward bilinear mapped convolution");
  m.def("mapped_conv_backward", &spherical::MappedConvBackward,
