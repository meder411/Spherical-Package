...
torch::Tensor WeightedMappedTransposedConvForward(
    ...
    torch::Tensor weight, torch::Tensor bias, int outputHeight, int outputWidth,
...
std::vector<torch::Tensor> WeightedMappedTransposedConvBackward(
    ...
    torch::Tensor bias, int kernel_size, InterpolationType interpolation) {
  CHECK_CONTIGUOUS(sample_map);
  CHECK_CONTIGUOUS(interp_weights);
  CHECK_CONTIGUOUS(input);
  CHECK_CONTIGUOUS(weight);
...
#endif
  {
    ...
    torch::Tensor grad_weight = cpu::WeightedMappedTransposedConvBackwardWeight(
        grad_output, sample_map, interp_weights, input, kernel_size,
        interpolation);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("weighted_mapped_transposed_conv_forward",
        &spherical::WeightedMappedTransposedConvForward, "Forward convolution");
  m.def("weighted_mapped_transposed_conv_backward",
        &spherical::WeightedMappedTransposedConvBackward,
