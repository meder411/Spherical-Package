...
template <typename T>
__host__ __device__ void XYZToSpherical(const T x, const T y, const T z, T &lon,
                                        T &lat) {
  lon = atan2(z, x);
...
template <typename T>
__host__ __device__ const T XToLongitude(const int64_t x, const int64_t width) {
  return 2 * M_PI * (T(x) - T(width) / 2) / T(width);
}

template <typename T>
__host__ __device__ const T YToLatitude(const int64_t y, const int64_t height) {
