...
__host__ __device__ inline void BilinearExtrapolation(
  ...
  const T D = y - y1;
  if (x1y1_valid_ptr) {
    atomic_add(x1y1_data_ptr, A * B * value);
  }
  if (x2y1_valid_ptr) {
    atomic_add(x2y1_data_ptr, A * C * value);
  }
  if (x1y2_valid_ptr) {
    atomic_add(x1y2_data_ptr, D * B * value);
  }
  if (x2y2_valid_ptr) {
    atomic_add(x2y2_data_ptr, D * C * value);
  }
}
...
template <typename T>
__host__ __device__ inline void Extrapolate1DNearest(const T data,
                                                     const T sample,
  ...
  if (sample_int_valid) {
    atomic_add(target_ptr + sample_int, data);
  }
}
...
__host__ __device__ inline const T Interpolate2DBilinear(const T x, const T y,
  ...
  const T x1y1_data = (x1_valid && y1_valid) ? data_ptr[y1 * width + x1] : T(0);
  const T x2y1_data = (x2_valid && y1_valid) ? data_ptr[y1 * width + x2] : T(0);
  const T x1y2_data = (x1_valid && y2_valid) ? data_ptr[y2 * width + x1] : T(0);
  const T x2y2_data = (x2_valid && y2_valid) ? data_ptr[y2 * width + x2] : T(0);
