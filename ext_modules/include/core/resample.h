...
__host__ __device__ void ResampleToMap2D(const T data, const T *sample_map_ptr,
  ...
  switch (interpolation) {
    ...
    case InterpolationType::BISPHERICAL:
...
template <typename T>
__host__ __device__ const T ResampleFromMap2D(
    const T *data_ptr, const T *sample_map_ptr, const int64_t height_im,
    const int64_t width_im, const InterpolationType interpolation) {
...
__host__ __device__ void ResampleToMap2DWeighted(
    ...
    const int64_t num_interp_pts, const InterpolationType interpolation,
