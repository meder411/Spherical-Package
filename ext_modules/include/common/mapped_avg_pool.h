...
__host__ __device__ void MappedAvgPool2D(
    ...
    const InterpolationType interpolation, T *out_data_ptr) {
...
__host__ __device__ void MappedAvgUnpool2D(
  ...
  for (int kern_idx = 0; kern_idx < kernel_size; kern_idx++) {
    ...
    core::ResampleToMap2D(value, sample_map_ptr + sample_map_idx + kern_idx * 2,
                          orig_height, orig_width, interpolation,
