#include "layers/distortion_layer.h"
#include "core/util.h"
#include "cpp/distortion.h"
...
torch::Tensor CreateDistortionSampleMap(const int64_t height,
  ...
  torch::Tensor sample_map = torch::zeros({height, width, 2}, params.options());
  ...
  SampleMapSelector<double>(sample_map, f, cx, cy, f, cx, cy, false, type,
                            params);
  ...
  SampleMapSelector<double>(sample_map, f, cx_p, cy_p, f, cx, cy, true, type,
                            params);
