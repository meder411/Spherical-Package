...
const ImageGrid ImageGrid::ParameterizedImageGrid(
  ...
  // return ImageGrid(height, width, f, cx, cy);

  // VertexDescriptor boundary_vd = VertexDescriptor(0);
  // for (const auto vd : this->_mesh.vertices()) {
  //   if (this->_mesh.is_border(vd)) {
  //     boundary_vd = vd;
  //     break;  // Not ideal...
  //   }
  // }

  // // Traverse the boundary of the mesh to create the polygon
  // auto start = boundary_vd;
  // do {
  //   // Add the boundary vertex to the boundary points list
  //   auto &pt = this->_mesh.point(boundary_vd);
  //   boundary_points.push_back(K::Point_2(pt.x(), pt.y()));

  //   // Gets the border half-edge incoming to this vertex
  //   auto he = this->_mesh.halfedge(boundary_vd);

  //   // Get the vertex from which the half-edge emanates (which will
  //   // also be a border vertex)
  //   boundary_vd = this->_mesh.source(he);

  // } while (boundary_vd != start);

  // // Grab the rest of the points as interior vertices
  // auto vertices_range = this->_mesh.vertices();
  // for (auto it = vertices_range.begin(); it != vertices_range.end(); it++) {
  //   if (!(this->_mesh.is_border(*it))) {
  //     auto &pt = this->_mesh.point(*it);
  //     interior_points.push_back(K::Point_2(pt.x(), pt.y()));
  //   }
  // }

  // Now create a new grid with (height, width) dimensions, but the interior
  // points will all be functions of the
}
