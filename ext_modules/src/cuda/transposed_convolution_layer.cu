...
torch::Tensor TransposedConvForward(torch::Tensor input, torch::Tensor weight,
                                    torch::Tensor bias, int kH, int kW, int dH,
  ...
  torch::Tensor columns = torch::zeros(
      {kW * kH * nOutputPlanes, inputHeight * inputWidth}, input.options());
  ...
  for (int b = 0; b < batchSize; b++) {
    ...
    if (input.dtype() == torch::kDouble) {
      ...
      cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, m, n, k, &alpha,
                  input.data_ptr<double>() + b * inputBatchStride, m,
                  weight.data_ptr<double>(), n, &beta,
                  columns.data_ptr<double>(), m);
    } else if (input.dtype() == torch::kFloat) {
      const float alpha = 1.0;
      const float beta  = 0.0;
      cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, m, n, k, &alpha,
                  input.data_ptr<float>() + b * inputBatchStride, m,
                  weight.data_ptr<float>(), n, &beta,
                  columns.data_ptr<float>(), m);
    } else {
    ...
    Col2Im2DLauncher(columns, nOutputPlanes, outputHeight, outputWidth,
                     inputHeight, inputWidth, kH, kW, padH, padW, dH, dW,
                     dilationH, dilationW, output[b]);
...
torch::Tensor TransposedConvBackwardInput(torch::Tensor grad_output,
                                          torch::Tensor weight,
  ...
  torch::Tensor columns =
      torch::zeros({kW * kH * nOutputPlanes, inputHeight * inputWidth},
                   grad_output.options());

  // For each elt in batch, do:
  const int64_t inputBatchStride = nInputPlanes * inputHeight * inputWidth;
  for (int b = 0; b < batchSize; b++) {
    Im2Col2DLauncher(grad_output[b], nOutputPlanes, outputHeight, outputWidth,
                     inputWidth, columns.size(1), kH, kW, padH, padW, dH, dW,
    ...
    if (grad_output.dtype() == torch::kDouble) {
      ...
      cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n, k, &alpha,
                  ...
                  &beta, input_grad.data_ptr<double>() + b * inputBatchStride,
                  m);
      ...
      cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n, k, &alpha,
                  ...
                  &beta, input_grad.data_ptr<float>() + b * inputBatchStride,
                  m);
    }
    CUDA_CHECK(cudaGetLastError())
  }
  ...
  for (int b = 0; b < batchSize; b++) {
    ...
    if (grad_output.dtype() == torch::kDouble) {
      ...
      cublasDgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, m, n, k, &alpha,
                  ...
                  input.data_ptr<double>() + b * inputBatchStride, k, &beta,
                  weight_grad.data_ptr<double>(), m);
    } else if (grad_output.dtype() == torch::kFloat) {
      const float alpha = 1.0;
      const float beta  = 1.0;
      cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, m, n, k, &alpha,
                  columns.data_ptr<float>(), k,
                  input.data_ptr<float>() + b * inputBatchStride, k, &beta,
                  weight_grad.data_ptr<float>(), m);
    }
    CUDA_CHECK(cudaGetLastError())
  }
