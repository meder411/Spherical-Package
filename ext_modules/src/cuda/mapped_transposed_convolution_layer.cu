...
torch::Tensor MappedTransposedConvForward(torch::Tensor input,
  ...
  for (int b = 0; b < batchSize; b++) {
    ...
    MappedCol2Im2DLauncher(columns, sample_map, nOutputPlanes, outputHeight,
                           outputWidth, inputWidth, columns.size(1),
                           kernel_size, interpolation, output[b]);
...
torch::Tensor MappedTransposedConvBackwardInput(
  ...
  const int64_t inputBatchStride = nInputPlanes * inputHeight * inputWidth;
  for (int b = 0; b < batchSize; b++) {
    MappedIm2Col2DLauncher(
        grad_output[b], sample_map, nOutputPlanes, outputHeight, outputWidth,
        inputWidth, columns.size(1), kernel_size, interpolation, columns);
